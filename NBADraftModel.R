...
new_bart_data <- new_bart_data[which(new_bart_data$yr != 'Jr' & new_bart_data$yr != 'Sr' & new_bart_data$yr != 'Fr'), ]
...
data_2023 <- new_bart_data[which(new_bart_data$year == 2024),]

new_bart_data <- new_bart_data[which(new_bart_data$year != 2024),]
...
xgb_model_soph <- xgb_model_upperclass

save(xgb_model_soph,file = "ncaa_molinar_soph_XGB.RData")
...
importance_matrix <- xgb.importance(feature_names = colnames(sparse_matrix_new), model = xgb_model_frosh)
